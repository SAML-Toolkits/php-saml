...
class OneLogin_Saml2_Settings
{
    ...
    public function checkSettings($settings)
    {
        ...
        if (!isset($settings['idp']) || empty($settings['idp'])) {
            ...
            if (isset($idp['artifactResolutionService'])
                && isset($idp['artifactResolutionService']['url'])
                && !empty($idp['artifactResolutionService']['url'])
                && !filter_var($idp['artifactResolutionService']['url'], FILTER_VALIDATE_URL)
            ) {
                $errors[] = 'idp_ars_url_invalid';
            }
        }
